              ...
              {filteredVehicles.map((vehicle) => (
                <tr key={vehicle.id} className="border-b border-gray-200 hover:bg-gray-100">
                  ...
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${vehicle.status === 'Inside' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      }`}>
                      {vehicle.status}
                    </span>
                  </td>
                  <td className="py-3 px-4">
                    {vehicle.status === 'Inside' && (
                      <button
                        ...
                        className={`font-bold py-1 px-3 rounded-full text-xs transition-colors duration-200 ${updatingId === vehicle.id
                            ? 'bg-gray-400 cursor-not-allowed'
                            : 'bg-blue-500 hover:bg-blue-700 text-white'
                          }`}
                      >
                        {updatingId === vehicle.id ? 'Updating...' : 'Record Exit'}
                      </button>
                    )}
                  </td>
                </tr>
